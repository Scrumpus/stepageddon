"""Writers that persist songs + charts to Postgres.

Single seam used by both the seed script and the generation router so the
JSONB shape stays identical regardless of source.
"""
...
from db.models import Chart, Song, SongSource
from modules.step_generator.schemas import Chart as PydanticChart
from modules.step_generator.schemas import StepType

from .sm_parser import ParsedChart, ParsedSong
...
async def _upsert_chart(
    session: AsyncSession, song_id: uuid.UUID, pc: ParsedChart
) -> None:
