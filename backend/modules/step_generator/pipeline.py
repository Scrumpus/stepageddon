...
from .audio_analysis import analyze_beats, analyze_onsets, analyze_energy, quantize_to_grid
...
class ChartGenerationPipeline:
    ...
    def generate_from_audio(audio_path: str, difficulty: str = 'intermediate') -> Chart:
        ...
        if config.use_onsets:
            ...
            onsets = quantize_to_grid(raw_onsets, tempo, config.grid_resolution)
            logger.info(f"Detected {len(raw_onsets)} onsets, quantized to {len(onsets)}")

        energy_sections = analyze_energy(y, sr)

        logger.info(f"Detected {len(beats)} beats at {tempo:.1f} BPM, duration {duration:.1f}s")

        logger.info(f"Generating {difficulty} chart...")
        generator = StepGenerator(config)
        chart = generator.generate_chart(beats, onsets, energy_sections, tempo, duration)
