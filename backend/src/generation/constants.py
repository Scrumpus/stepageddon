"""
Difficulty presets for chart generation.

Each preset defines the knobs for density, timing, and patterns. The ML
inference path conditions on the named difficulty; the per-knob fields stay
on :class:`DifficultyConfig` for compatibility with legacy callers.
"""

from src.charts.schemas import DifficultyConfig
...
DIFFICULTY_PRESETS = {
    "beginner": DifficultyConfig(
        ...
        max_notes_under_hold=0,     # holds sit in rests only
    ...
    "medium": DifficultyConfig(
        ...
        max_notes_under_hold=2,     # a couple of slow taps under a hold
    ...
    "challenge": DifficultyConfig(
        ...
        max_notes_under_hold=12,    # full hold-under-stream
