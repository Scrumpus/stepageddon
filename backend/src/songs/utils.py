"""Song helpers: API URL formatting, summary projection, and StepMania parsing.

The parser produces two views over a single .sm/.ssc parse:

* :func:`parse_sm_file` — flat per-event ``Note`` records (used by ML training
  data prep).
* :func:`parse_sm` — grouped :class:`Step` records with hold durations (used
  by the seed script and runtime DB writer).
"""
...
def _to_summary(song: Song) -> SongSummaryDTO:
